...
class MultiFrameMainActivity : AppCompatActivity() {
...
//        device_id = "cd7598568b48f67"

//        device_id = "e244e6cc9c428e50"
...
//                    web.setWebViewClient(WebViewClient())
            ...
            for (i in 0..multiframe_items.size-1){
                if(multiframe_items.get(i) != null && multiframe_items.get(i).size>0){
                    if(current_size_list[i] < multiframe_items.get(i).size){
                        ...
                        if(multiframe_items.get(i)[current_size_list[i]].type == Constant.CONTENT_IMAGE
                        ...
                        if(multiframe_items.get(i)[current_size_list[i]].type == Constant.CONTENT_VIDEO) playVideo(i,multiframe_items.get(i).size,multiframe_items[i].get(current_size_list[i]).duration)
                        if(multiframe_items.get(i)[current_size_list[i]].type == Constant.CONTENT_WEB) showWebView(i,multiframe_items.get(i)[current_size_list[i]])
                    }
                }
            }
        }
    ...
    private fun showWebView(pos: Int, item: Item) {
        Log.d(TAG, "showWebView: item_size - ${multiframe_items[pos].size}")
        Log.d(TAG, "showWebView: time - ${Utility.getCurrentdate()}")
...
//                layout_list[pos].webView?.loadUrl("https://www.facebook.com/")
            ...
            CoroutineScope(Dispatchers.IO).launch {
                    ...
                    withContext(Dispatchers.Main) {
                        ...
                        Log.d(TAG, "showWebView: item_size - ${items.size}")
                        if(reload.equals("n") && multiframe_items[pos].size==1){
                        }else{
    ...
    private fun nextPlay(pos:Int,from:Int){
        ...
        if(current_size_list[pos] < multiframe_items.get(pos).size){
            ...
            if(multiframe_items.get(pos)[current_size_list[pos]].type == Constant.CONTENT_IMAGE
            ...
            if(multiframe_items.get(pos)[current_size_list[pos]].type == Constant.CONTENT_VIDEO) playVideo(pos,multiframe_items.get(pos).size,multiframe_items[pos].get(current_size_list[pos]).duration)
            if(multiframe_items.get(pos)[current_size_list[pos]].type == Constant.CONTENT_WEB) showWebView(pos,multiframe_items.get(pos)[current_size_list[pos]])
        }else{
...
//        Toast.makeText(this, msg, Toast.LENGTH_LONG).show()
    ...
    fun refreshPage(from: String) {
        ...
        if(from.equals(Constant.REFRESH_FROM_NODEVICE) && dialog==null){
            ...
            finish()
            startActivity(Intent(this,MirrorMultiFrameMainActivity::class.java))
